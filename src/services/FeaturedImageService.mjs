...
export class FeaturedImageService {
  ...
  async generate({ prompt, aspectRatio, imageSize, model, name = "featured", useStyle = true }) {
    ...
    const { data, mimeType, model: usedModel } = await this.imageClient.generateImage({
      prompt: fullPrompt,
      aspectRatio,
      imageSize,
      model,
   ...
   * @returns {Promise<{mode: string, blockId: string, fileUploadId: string}>}
   * @throws {Error} replace_first 인데 페이지에 이미지 블록이 없는 경우
   */
  async attachToNotion({ pageId, filePath, caption, mode = "prepend" }) {
    if (!caption?.trim()) {
    ...
    if (mode === "replace_first") {
      ...
      return { mode, blockId: block.id, fileUploadId };
    }
