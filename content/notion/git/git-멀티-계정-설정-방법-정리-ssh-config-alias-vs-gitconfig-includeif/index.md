...
description: "Git 멀티 계정 환경에서 ~/.ssh/config 알리아스로 SSH 키를 분기하고 ~/.gitconfig includeIf로 폴더별 커밋 사용자 정보를 자동 분리하는 방법을 정리했습니다. 계정 혼선과 인증 실수를 줄이는 기준을 확인하세요."
categories:
  - "git"
tags:
  - "git"
  - "github"
  - "gitlab"
date: 2025-05-14T08:41:00.000Z
lastmod: 2026-02-27T09:05:00.000Z
toc: true
draft: false
images:
...
단점은 기존 원격 URL을 알리아스 기반 URL로 변환해서 사용해야 한다.


> git@github.com:user/repo.git 같은 주소를 알리아스 기반 URL  
...
| 구분    | ~/.ssh/config 호스트 별칭(알리아스)                                     | ~/.gitconfig includeIf (폴더 기준)                                            |
| ----- | -------------------------------------------------------------- | ------------------------------------------------------------------------- |
...
| 단점    | 기존 remote URL을 알리아스 URL로 수동 변환해야 한다.                           | 폴더 구조를 먼저 정해야 한다. 규칙이 꼬이면 의도와 다른 설정이 적용될 수 있다.                            |
