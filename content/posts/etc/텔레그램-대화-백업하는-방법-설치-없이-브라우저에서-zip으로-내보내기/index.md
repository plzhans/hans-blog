...
lastmod: 2026-09-03T07:25:00.000Z
toc: true
draft: false
images:
...
![설치 없이 브라우저에서 텔레그램 대화를 zip으로 내보내는 백업 도구를 나타난 대표 이미지](./assets/1_3a822a0f-7e83-8070-a296-d9dffad002b5.png)
...
File System Access API(`showSaveFilePicker`)가 있으면 압축한 청크를 **디스크로 바로 흘려보낸다.** 없으면(Firefox나 Safari) 모아서 Blob으로 내려받고 그렇다고 화면에 알린다. 이 피커는 사용자 제스처가 있어야 뜬다. 그래서 <strong>클릭 핸들러의 맨 처음</strong>에 부른다. 내보내기가 시작된 뒤에 부르면 제스처가 이미 소모돼 거부당한다.
...
`index.html`은 **파일 하나로 완결된다.** 스타일이 전부 인라인이라 인터넷이 없어도 열리고 이 도구가 없어도 열린다. 그리고 **스크립트가 없다.** 증거로 제시하는 문서에 코드가 들어 있으면 "그때 그렇게 렌더된 것뿐"이라는 여지가 생기기 때문이다. 언제 열어도 같은 것을 보여준다.
