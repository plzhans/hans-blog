...
lastmod: 2026-09-23T11:18:00.000Z
toc: true
draft: false
images:
...
Deuxfleurs 라는 프랑스의 비영리 호스팅 단체가 만든 S3 호환 오브젝트 스토리지다. 프로젝트가 스스로를 소개하는 문장이 "데이터센터 밖에서도 돌릴 만큼 안정적인 S3 오브젝트 스토어" 인데, 설계 의도가 그 한 줄에 들어 있다. 전용 백본이 없고 회선도 느린 집이나 사무실의 서버를 여럿 묶어 쓰는 상황을 전제로 만들어졌다.


특징을 셋만 꼽으면 이렇다. 외부 데이터베이스 없이 바이너리 하나로 돌아간다. 노드마다 존을 지정해 복제본을 지리적으로 흩어놓는다. 그리고 S3 API 를 구현해서 기존 SDK 와 도구가 그대로 붙는다.


Rust 로 쓰였고 라이선스는 AGPL-3.0 다.

- 메인 저장소: [git.deuxfleurs.fr/Deuxfleurs/garage](https://git.deuxfleurs.fr/Deuxfleurs/garage)
- GitHub 미러: [deuxfleurs-org/garage](https://github.com/deuxfleurs-org/garage)
...
Garage 는 S3 API 를 말하지만 S3 가 아니다. 


이 세 가지만 알고 시작하면 나머지는 순조롭다.

- 버킷을 만들기 전에 노드에 역할을 줘야 한다
- 권한은 키 단위로만 존재한다.
- 설정 파일 없이는 기동조차 하지 않는다.

바이너리 하나에 설정 파일 한 장, 필수 항목은 다섯 개뿐이다. 도커 컴포즈 파일 열다섯 줄로 S3 호환 스토리지가 생기고 `aws` CLI 가 그대로 붙는다.


여기까지가 바닥이다. 실제로 쓰려면 도메인을 붙이고 HTTPS 를 씌워야 하는데, Garage 자신은 TLS 를 하지 않으므로 앞단이 필요하다. 


공개 파일을 웹에 내보내는 것도 S3 API 가 아니라 별도 엔드포인트가 맡는다. 
