...
CLI でやっていたことがブラウザに移っただけで、動作は同じです。Assign と Apply が分かれているのも、キーとバケットを別々に作って権限でつなぐのも CLI と変わりません。webui は admin API をラップした画面なので、Garage 側の概念を知っていれば迷うところはありません。逆に概念を知らずに画面から入ると、Assign だけして Apply を忘れる、といった形で詰まります。

留意点は 2 つです。公式コンソールではなくサードパーティなので、Garage 本体と更新サイクルが異なります。そして **認証はデフォルトでオフです。** `AUTH_USER_PASS` を設定しなければ、3909 に到達できる人は誰でも管理者になります。内部ネットワークであっても、開けておく理由はありません。
