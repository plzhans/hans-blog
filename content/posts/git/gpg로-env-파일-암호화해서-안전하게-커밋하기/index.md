...
description: ".env 파일을 Git에 커밋해야 할 때 GPG로 암호화해 비밀값 노출을 막는 방법을 정리했습니다. .gitignore 처리부터 gpg encrypt/decrypt 명령과 recipient 기본 설정까지 적용해 안전한 저장소 운영을 시작하세요."
categories:
  - "git"
tags:
  - "git"
...
![GPG로 .env 파일을 암호화해 비밀값 노출 없이 Git에 커밋하는 흐름을 나타난 대표 이미지](./assets/1_30922a0f-7e83-803e-8671-fa8de7b1660a.png)
...
이 글에서는 <strong>비밀 정보가 포함된 파일을 커밋할 때 내용을 암호화해 은닉하는 방법</strong>을 정리합니다.


대표적인 접근은 여러 가지가 있지만 여기서는 <strong>GPG를 직접 사용하는 방식</strong>만 다룹니다.
...
- <strong>이미 과거 커밋에 포함</strong>된 적이 있다면 `.gitignore`에 추가해도 **과거 커밋에서 비밀값을 열람할 수 있습니다.**
- 완전히 없애려면 비밀값을 <strong>폐기/재발급</strong>하고 git 커밋을 <strong>재작성</strong>해야 합니다.
...
- `gpg --encrypt`는 기존 파일을 업데이트하는 개념이 아니라, <strong>매번 출력 파일을 새로 생성</strong>합니다.
