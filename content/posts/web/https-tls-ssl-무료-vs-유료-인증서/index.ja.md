...
description: "HTTPS構築に必要なTLS/SSL証明書の発行・検証フローと無料(ACME/Let's Encrypt)・有料・クラウド管理型オプションの比較を整理しました。更新自動化と終端ポイントの最小化戦略でコストと運用リスクを抑える方法を確認してください。"
tags:
  - "acme"
  - "https"
  - "ssl"
...
![HTTPSに必要なTLS/SSL証明書の無料・有料・クラウドマネージド型の選択肢を比較する内容を表したカバー画像](./assets/1_2fe22a0f-7e83-805c-bf08-d71559e37b4a.png)
...
### 参考: ローカルDNSキャッシュによる確認遅延


ドメインのDNSレコードを変更した直後なのに、ブラウザで依然として以前のサーバーにつながっているように見える場合、実際の設定問題ではなく、PCに残っているローカルDNSキャッシュが原因のことがあります。この場合キャッシュを消すとすぐに解決することが多いです。方法は[Windows/Linux/macOS DNSキャッシュのクリア](../6-flush-dns-cache/)を参照してください。
