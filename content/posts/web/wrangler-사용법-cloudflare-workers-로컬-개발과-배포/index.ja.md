...
![ローカルで wrangler dev により動かした Worker ランタイムが、wrangler deploy でそのままエッジへ上がっていく過程](./assets/1_3d822a0f-7e83-81f6-b43f-ee90a3347422.jpg)
...
**一つの設定に両方を詰め込むのも答えではありません。**DOMとWorkersの型を一緒に入れると、`Request`・`Response`・`caches` のように**両側に同じ名前が違う形で存在するもの**が混ざり、見当違いの型が当たってしまいます。
...
| -------- | ------------------ | ----------------------------- |
| 何なのか     | フロントの**開発サーバー** | プロダクション**ランタイムの複製**      |
...
`wrangler deploy --name X` はXがなければ**作り**、あれば上書きします。便利に見えますが、**名前を間違えたデプロイが静かに「成功」します。** 見当違いのWorkerが新しく作られ、肝心の見ているサイトは変わりません。
