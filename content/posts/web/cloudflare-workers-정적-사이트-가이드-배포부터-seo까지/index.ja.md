...
![Cloudflare Workers 静的サイトシリーズが、ホスティング・ローカル開発・キャッシュ・SEO の4編につながる構成](./assets/1_3d822a0f-7e83-818c-8060-c230fe8eebe7.jpg)
...
**`wrangler dev` はフロントエンドの開発サーバーではありません。**プロダクションと同じランタイム（`workerd`）をローカルに立ち上げるものなので、ソースではなく**ビルド成果物**を読み込みます。これを知らないと「確かに直したのになぜそのままなのか」を繰り返すことになります。
...
かける方法は2つですが、**選ぶ基準があります。**そしてエッジキャッシュが**データセンターごとに別**という点は、知ってしまえば当然ですが、知らなければ「なぜキャッシュが当たらないのか」としばらく悩むことになります。
