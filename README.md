...
GitHub Actions 스케줄(`ga4-popular-posts.yml`)로 주기적으로 실행됩니다.
> 자세한 내용은 [`docs/ga4.md`](docs/ga4.md)를 참고하세요.

GA4 API로 `post_slug` 커스텀 이벤트를 집계하여 최근 인기 게시글 데이터를 추출하고 게시글 정보와 매칭하여 Hugo 페이지를 생성합니다.
- [한국어 인기글 데이터](data/popular/ko.json)
- [영어 인기글 데이터](data/popular/en.json)
- [일본어 인기글 데이터](data/popular/ja.json)
...
Hugo의 Related Content 기능으로 게시글 하단에 관련글을 자동 추천합니다.

게시글 하단에 두 섹션이 순서대로 표시됩니다.
1. **카테고리의 다른 글** — 같은 카테고리 최신글 최대 5개
2. **추천 글** — 태그·카테고리·날짜 유사도 기반 최대 5개 (1번과 중복 제외)

설정은 [`hugo/hugo.toml`](hugo/hugo.toml)의 `[related]` 섹션에서 관리합니다.

> 자세한 내용은 [`docs/hugo-related.md`](docs/hugo-related.md)를 참고하세요.
